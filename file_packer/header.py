...
class FilePackHeader(object):
    ...
    def __init__(self, version, filenames, buffer_sizes, base_path=""):
        ...
        if self.num_files != len(buffer_sizes):
            raise ValueError(
                "'buffer_sizes' and 'filenames' must contain an equal number of elements "
        ...
        if len(base_path) > self.__MAX_BASE_PATH_LENGTH:
            raise ValueError(
                "Base path cannot be longer than {} characters. The provided base path in {} "
    ...
    def __str__(self):
        return "Format version: {}\n" "Number of files: {}\n" "Base path: {}".format(
            self.version, self.num_files, self.base_path
        )
    ...
    def _pad_filenames(self):
        for filename in self.filenames:
            if len(filename) > self.__MAX_FILENAME_LENGTH:
                raise ValueError(
                    "Filenames cannot be longer than {}. The filename {} has a length of {}".format(
    ...
    def read_from_file_handle(cls, ifile):
        ...
        # read base path
        base_path = struct.unpack(
            "%ds" % cls.__MAX_BASE_PATH_LENGTH, ifile.read(cls.__MAX_BASE_PATH_LENGTH)
        )[0]
        base_path = base_path.decode("utf-8").rstrip()

        # read image file names
        filenames_block_length = num_images * cls.__MAX_FILENAME_LENGTH
        filenames = struct.unpack(
            "%ds" % filenames_block_length, ifile.read(filenames_block_length)
        )[0]
        filenames = filenames.decode("utf-8")
        filenames = [
            filenames[i : i + cls.__MAX_FILENAME_LENGTH].rstrip()
            for i in range(0, len(filenames), cls.__MAX_FILENAME_LENGTH)
        ]
